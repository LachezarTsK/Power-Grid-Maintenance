...
function processQueries(numberOfPowerStations: number, connections: number[][], queries: number[][]): number[] {
    const operationalStations: boolean[] = new Array(numberOfPowerStations + 1).fill(true);
    const unionFind = new UnionFind(numberOfPowerStations);
    for (let connection of connections) {
        unionFind.joinByRank(connection[0], connection[1]);
    }

    const parentToOperationalStations: Map<number, OperationalStations> = createParentToOperationalStations(unionFind, operationalStations, numberOfPowerStations);

    return assignStationsDuringMaintenance(queries, unionFind, parentToOperationalStations);
};

function createParentToOperationalStations(unionFind: UnionFind, operationalStations: boolean[], numberOfPowerStations: number): Map<number, OperationalStations> {
    ...
    for (let station = 1; station <= numberOfPowerStations; ++station) {
        ...
        if (!parentToOperationalStations.has(parent)) {
            parentToOperationalStations.set(parent, new OperationalStations(operationalStations));
        }
...
class OperationalStations {
    ...
    operationalStations: boolean[];
    minHeapForStationID = new PriorityQueue<number>((x, y) => x - y);

    constructor(operationalStations: boolean[]) {
    ...
    removeStation(station: number): void {
        this.operationalStations[station] = false;
    }

    getStationToTakeOverDuringMaintenanceCheck(station: number): number {
        if (this.operationalStations[station]) {
            return station;
        }

        while (!this.minHeapForStationID.isEmpty() && !this.operationalStations[this.minHeapForStationID.front()]) {
            this.minHeapForStationID.dequeue();
        }
        if (this.minHeapForStationID.isEmpty()) {
